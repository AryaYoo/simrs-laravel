...
class CetakLaporanOperasiController extends Controller
{
    ...
    public function cetakKhanza(Request $request, $no_rawat, $tanggal = null)
    {
        $no_rawat = str_replace('-', '/', $no_rawat);
        if ($tanggal) {
            $tanggal = str_replace('_', ' ', $tanggal);
        ...
        if ($operasi) {
            $roles = [
                ...
                'instrumen', 'perawaat_resusitas', 'asisten_anestesi', 'asisten_anestesi2',
                'bidan', 'bidan2', 'bidan3', 'perawat_luar',
                'omloop', 'omloop2', 'omloop3', 'omloop4', 'omloop5'
            ];
            foreach ($roles as $role) {
                if (!empty($operasi->$role)) {
                    $staff[$role] = LaporanOperasiRepository::resolveStaffName($operasi->$role);
                } else {
                    $staff[$role] = '-';
                }
            }
        ...
        return view('modul.casemix.kustom-laporan-operasi.cetak-khanza', compact('laporan', 'operasi', 'staff', 'setting', 'preSurgical'));
    }
}