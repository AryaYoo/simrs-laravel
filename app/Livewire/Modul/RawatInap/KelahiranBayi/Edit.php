...
#[Layout('layouts.app', ['title' => 'Edit Data Kelahiran Bayi - SIMRS LaraLite'])]
#[Title('Edit Data Kelahiran Bayi - SIMRS LaraLite')]
class Edit extends Component
{
    ...
    {
        $bayi = \App\Models\PasienBayi::with(['pasien', 'pegawai'])->find($no_rkm_medis);
        
        if (!$bayi) {
            abort(404, 'Data Pasien Bayi tidak ditemukan.');
        }

        // Map data to properties
        $this->no_rkm_medis = $bayi->no_rkm_medis;
        $this->nm_pasien = $bayi->pasien->nm_pasien ?? '';
        $this->nm_ibu = $bayi->pasien->nm_ibu ?? '';
        $this->jk = $bayi->pasien->jk ?? 'L';
        $this->tgl_lahir = $bayi->pasien->tgl_lahir ?? now()->format('Y-m-d');
        $this->tgl_daftar = $bayi->pasien->tgl_daftar ?? now()->format('Y-m-d');
        ...
        $this->panjang_badan = $bayi->panjang_badan;
        $this->berat_badan = $bayi->berat_badan;
        $this->lingkar_dada = $bayi->lingkar_dada;
        $this->lingkar_kepala = $bayi->lingkar_kepala;
        $this->lingkar_perut = $bayi->lingkar_perut;
        $this->jam_lahir = $bayi->jam_lahir;
        
        $this->penyulit_kehamilan = $bayi->penyulit_kehamilan ?? '';
        $this->proses_lahir = $bayi->proses_lahir;
        $this->anakke = $bayi->anakke;
        $this->kelahiran_ke = $bayi->kelahiran_ke ?? '';
        $this->g = $bayi->g;
        $this->p = $bayi->p;
        $this->a = $bayi->a;
        ...
        $this->resusitas = $bayi->resusitas;
        $this->obat_diberikan = $bayi->obat_diberikan;
        $this->mikasi = $bayi->mikasi;
        $this->mikonium = $bayi->mikonium;
    }
    ...
    {
        return view('livewire.modul.rawat-inap.kelahiran-bayi.edit');
    }
}
