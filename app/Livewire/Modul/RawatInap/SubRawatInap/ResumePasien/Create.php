...
class Create extends Component
{
    ...
    // ─────────────────────────────────────────────
    // Auto-Fill Methods (Optimized)
    // ─────────────────────────────────────────────

    public function autoFillSOAP($field, $column)
    {
        $allData = \App\Models\PemeriksaanRanap::where('no_rawat', $this->no_rawat)
    ...
    {
        $dr = \App\Models\RawatInapDr::with('jnsPerawatan')->where('no_rawat', $this->no_rawat)->get();
        $pr = \App\Models\RawatInapPr::with('jnsPerawatan')->where('no_rawat', $this->no_rawat)->get();
        $drpr = \App\Models\RawatInapDrpr::with('jnsPerawatan')->where('no_rawat', $this->no_rawat)->get();

        $allTindakan = $dr->concat($pr)->concat($drpr)
    ...
    {
        $allObat = \App\Models\DetailPemberianObat::with('barang')
    ...
    public function toggleSelectAll()
    {
        if ($this->targetAttachColumn == 'lab_hasil') {
            ...
            $obatPulang = $this->obatPulangData();
    ...
    {
        return \App\Models\DetailPeriksaLab::with('template')
            ->where('no_rawat', $this->no_rawat)
    ...
    {
        $dr = \App\Models\RawatInapDr::with(['jnsPerawatan', 'dokter', 'petugas'])->where('no_rawat', $this->no_rawat)->get();
        $pr = \App\Models\RawatInapPr::with(['jnsPerawatan', 'dokter', 'petugas'])->where('no_rawat', $this->no_rawat)->get();
        $drpr = \App\Models\RawatInapDrpr::with(['jnsPerawatan', 'dokter', 'petugas'])->where('no_rawat', $this->no_rawat)->get();
        
        return $dr->concat($pr)->concat($drpr)->sortByDesc(fn($t) => $t->tgl_perawatan . ' ' . $t->jam_rawat);
    }

    #[Computed]
    public function obatData()
    {
        return \App\Models\DetailPemberianObat::with('barang')
            ->where('no_rawat', $this->no_rawat)
    ...
    {
        return \App\Models\PemeriksaanRanap::where('no_rawat', $this->no_rawat)
            ->orderBy('tgl_perawatan', 'desc')
    ...
    {
        // Reload basic regPeriksa relations without heavy eager loading
        $this->regPeriksa = RegPeriksa::with([
            'pasien', 'dokter', 'kamarInap.kamar.bangsal'
        ])->findOrFail($this->no_rawat);
