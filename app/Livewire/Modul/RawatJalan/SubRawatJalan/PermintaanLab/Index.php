...
namespace App\Livewire\Modul\RawatJalan\SubRawatJalan\PermintaanLab;

use App\Models\RegPeriksa;
use App\Models\TemplateLaboratorium;
use App\Models\Dokter;
use App\Repositories\RawatJalan\PermintaanLabRepository;
...
class Index extends Component
{
    ...
    public function mount($no_rawat)
    {
        ...
        $this->regPeriksa = RegPeriksa::with(['pasien', 'dokter', 'poliklinik'])->where('no_rawat', $this->no_rawat)->first();
    ...
    public function toggleTest($kd_jenis_prw)
    {
        if (in_array($kd_jenis_prw, $this->selectedTests)) {
            $this->selectedTests = array_diff($this->selectedTests, [$kd_jenis_prw]);
        } else {
            $this->selectedTests[] = $kd_jenis_prw;
        }
        $this->updatedSelectedTests($this->selectedTests);
    }

    public function toggleDetail($id_template)
    {
        $id_template = (string)$id_template;
        if (in_array($id_template, $this->selectedDetails)) {
            $this->selectedDetails = array_diff($this->selectedDetails, [$id_template]);
        } else {
            $this->selectedDetails[] = $id_template;
        }
    }
    ...
    {
        return view('livewire.modul.rawat-jalan.sub-rawat-jalan.permintaan-laboratorium.index', [
