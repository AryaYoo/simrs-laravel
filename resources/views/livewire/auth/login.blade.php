    ...
    <div class="auth-card p-7">
        <div class="flex flex-col gap-2 text-center">
            <div class="flex flex-col items-center mb-2">
                <h1 class="font-extrabold" style="color: #1B9C85; font-size: 1.75rem; line-height: 1.2; letter-spacing: 0.05em;">LARALITE</h1>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-6">
            ...
            <flux:input name="username" :label="__('Username')" :value="old('username')" required autofocus
                ...
                class="bg-zinc-50 border-indigo-900/10 focus:border-teal-600 dark:bg-zinc-800 dark:border-zinc-700"
                style="background-color: rgba(232, 246, 239, 0.3);" />
            ...
            <div class="relative">
                ...
                @if (Route::has('password.request'))
                    <flux:link class="absolute top-0 text-sm end-0 font-medium" :href="route('password.request')"
                        wire:navigate style="color: #1B9C85;">
                        {{ __('Forgot?') }}
                    </flux:link>
                @endif
            </div>
            ...
            <div class="flex items-center justify-end">
                <flux:button type="submit"
                    ...
                    style="background-color: #1B9C85; box-shadow: 0 10px 15px -3px rgba(27, 156, 133, 0.2);">
                    {{ __('Log in') }}
                </flux:button>
            </div>
        </form>
