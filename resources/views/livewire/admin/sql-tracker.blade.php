<div class="flex flex-col gap-6 pb-8" x-data="{ showDetail: @entangle('detailModalOpen') }">
    {{-- Header / Breadcrumb --}}
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {{-- Filter User --}}
            <div>
                <flux:select wire:model.live="user" label="Filter User / Pegawai">
            ...
            {{-- Filter Date --}}
            <div>
                <flux:input type="date" wire:model.live="date" label="Filter Tanggal" />
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-neutral-50 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-300 font-semibold">
                    ...
                    @forelse($mappedLogs as $log)
                        <tr class="hover:bg-neutral-50/50 dark:hover:bg-neutral-900/50 transition-colors">
                            ...
                            <td class="p-4 text-center">
                                <flux:button size="sm" variant="ghost" wire:click="showDetail('{{ $log['tanggal'] }}', '{{ $log['ip'] . ' ' . addslashes($log['raw_sql']) }}', '{{ $log['usere'] }}')">
                                    Detail
                                </flux:button>
                            </td>
                        </tr>
    ...
    {{-- Detail SQL Modal (Alpine.js - SOP #6) --}}
            ...
            @if($selectedLog)
                <div class="flex-1 overflow-y-auto space-y-4 pr-1 text-sm">
                    <div class="grid grid-cols-2 gap-4 bg-neutral-50 dark:bg-neutral-950 p-4 rounded-xl border border-neutral-100 dark:border-neutral-800/80">
                        <div>
                            ...
                            <span class="font-medium text-neutral-700 dark:text-neutral-300">
                                {{ Carbon\Carbon::parse($selectedLog['tanggal'])->format('d F Y (H:i:s)') }}
                            </span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span class="font-medium text-neutral-700 dark:text-neutral-300 font-mono text-xs">
                                {{ $selectedLog['ip'] }}
                            </span>
                        </div>
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-2">
                                @if($selectedLog['action'] === 'INSERT')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-green-50 text-green-700 border border-green-200 dark:bg-green-950/20 dark:text-green-400 dark:border-green-900/30">INSERT</span>
                                @elseif($selectedLog['action'] === 'UPDATE')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/30">UPDATE</span>
                                @elseif($selectedLog['action'] === 'DELETE')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900/30">DELETE</span>
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-neutral-100 text-neutral-700 border border-neutral-200">{{ $selectedLog['action'] }}</span>
                                @endif
                                <span class="font-mono text-xs font-bold text-[#4C5C2D] dark:text-[#8CC7C4]">
                                    {{ $selectedLog['table'] }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative bg-neutral-950 text-neutral-200 font-mono text-xs p-4 rounded-xl border border-neutral-800 overflow-x-auto leading-relaxed whitespace-pre-wrap select-all">
                            {{ $selectedLog['raw_sql'] }}
                        </div>
                    </div>
                </div>
