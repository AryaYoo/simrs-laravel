<div class="flex flex-col gap-6 pb-8">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('modul.casemix-rawat-jalan.index') }}" wire:navigate class="flex items-center justify-center w-10 h-8 rounded-md bg-[#4C5C2D] transition-colors hover:bg-[#3d4b24] shadow-sm">
            ...
            <div>
                <nav class="text-xs text-neutral-400 mb-0.5">
                    ...
                    <a href="{{ route('modul.casemix-rawat-jalan.index') }}" wire:navigate class="hover:underline">Casemix Rawat Jalan</a>
                    <span class="mx-1">/</span>
                    <span>Resume Medis</span>
                </nav>
                <h1 class="text-xl font-bold text-neutral-800 dark:text-neutral-100">Data Resume Medis Pasien CASEMIX</h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 p-5 shadow-sm overflow-hidden relative">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="px-4 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-900/50 border border-neutral-100 dark:border-neutral-700 flex flex-col items-center">
                    <span class="text-[10px] uppercase font-bold text-neutral-400 leading-none mb-1">Poli/Unit</span>
                    <span class="text-sm font-bold text-neutral-700 dark:text-neutral-300">{{ $regPeriksa->poliklinik->nm_poli ?? '-' }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-900/20 flex items-center justify-between">
            ...
            <a href="{{ route('modul.casemix-rawat-jalan.resume.form', str_replace('/', '-', $no_rawat)) }}" wire:navigate>
                ...
                <tbody class="divide-y divide-neutral-100 dark:divide-neutral-700">
                    @forelse($resumes as $resume)
                        <tr class="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/10 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold text-neutral-800 dark:text-neutral-100 truncate block max-w-[200px] uppercase">{{ $resume->regPeriksa->pasien->nm_pasien ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs font-medium text-neutral-600 dark:text-neutral-400">{{ $resume->regPeriksa->dokter->nm_dokter ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs text-neutral-500">{{ $resume->regPeriksa->poliklinik->nm_poli ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('modul.casemix-rawat-jalan.resume.form', str_replace('/', '-', $resume->no_rawat)) }}" wire:navigate
                                       title="Edit Resume (CASEMIX)" class="p-2 rounded-lg bg-neutral-100 dark:bg-neutral-700 text-neutral-500 hover:bg-[#4C5C2D]/10 hover:text-[#4C5C2D] dark:hover:text-[#8CC7C4] transition-all">
