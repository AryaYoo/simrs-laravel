<div class="flex flex-col gap-6 pb-8">
    ...
    <div class="flex flex-col gap-1">
        <div class="flex items-center gap-3">
            <a href="{{ route('modul.rawat-inap.perawatan-tindakan', str_replace('/', '-', $regPeriksa->no_rawat)) }}" wire:navigate class="flex items-center justify-center w-10 h-8 rounded-md bg-[#4C5C2D] transition-colors hover:bg-[#3d4b24] shadow-sm">
            ...
            <div>
                <nav class="text-xs text-neutral-400 mb-0.5">
                    ...
                    <span>Observasi Induksi Persalinan</span>
                </nav>
                <h1 class="text-xl font-bold text-neutral-800 dark:text-neutral-100">Observasi Induksi Persalinan</h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm overflow-hidden flex flex-col mt-2 relative p-4">
        <div class="flex items-center justify-between mb-4 px-2">
            <h3 class="text-sm font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-wider flex items-center gap-2">
                ...
                Daftar Observasi Induksi Persalinan
            </h3>
        ...
        <div class="overflow-x-auto">
            <flux:table class="whitespace-nowrap mt-2">
                <flux:table.columns>
                    ...
                    <flux:table.column>Cairan</flux:table.column>
                    <flux:table.column>Dosis</flux:table.column>
                    <flux:table.column>HIS</flux:table.column>
                    <flux:table.column>DJJ</flux:table.column>
                    <flux:table.column>Keterangan</flux:table.column>
                    <flux:table.column>Bidan</flux:table.column>
                ...
                <flux:table.rows>
                    @forelse ($observasis as $obs)
                        <flux:table.row>
                            ...
                            <flux:table.cell class="text-neutral-600 dark:text-neutral-400 truncate max-w-xs">
                                {{ $obs['keterangan'] ?? '-' }}
                            </flux:table.cell>
                            <flux:table.cell class="text-xs text-neutral-600 dark:text-neutral-400">
                                <span class="font-mono text-neutral-500 block mb-0.5">{{ $obs['nip'] }}</span>
                                {{ $obs['petugas']['nama'] ?? '-' }}
                            </flux:table.cell>
                            <flux:table.cell>
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="13">
                                <div class="flex flex-col items-center justify-center py-12 text-center">
                                    ...
                                    <p class="text-neutral-500 mt-1 max-w-md">Belum ada catatan observasi induksi persalinan yang ditambahkan untuk pasien ini.</p>
                                </div>
                            </flux:table.cell>
                        </flux:table.row>
                    @endforelse
    ...
    <div x-data="{ 
        ...
        <div x-show="open"
             ...
             class="relative w-full max-w-4xl bg-white dark:bg-neutral-800 rounded-3xl shadow-2xl overflow-hidden border border-neutral-200 dark:border-neutral-700">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-neutral-800 dark:text-neutral-100">
                            {{ $isEditMode ? 'Edit Observasi Induksi Persalinan' : 'Input Observasi Induksi Persalinan' }}
                        </h3>
                        <p class="text-xs text-neutral-500">Lengkapi data observasi dan pemberian obat induksi.</p>
                    </div>
                </div>
            ...
            <div class="p-8">
                    ...
                    <div class="md:col-span-3">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <div class="flex items-center gap-1.5 mt-2">
                                <flux:checkbox wire:model="autoTime" id="autoTimeObsInduksi" />
                                <label for="autoTimeObsInduksi" class="text-xs font-medium text-neutral-600 dark:text-neutral-400 cursor-pointer">Waktu Otomatis</label>
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <flux:input wire:model="obat" label="Obat" placeholder="Ex: Oxytocin" />
                    <flux:input wire:model="cairan" label="Cairan *" placeholder="Ex: RL 500cc" required />
                    <flux:input wire:model="dosis" label="Dosis" placeholder="Ex: 5 IU" />
                </div>

                {{-- HIS, DJJ, Keterangan --}}
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                    <div class="md:col-span-3">
                        <flux:input wire:model="his" label="HIS" placeholder="Ex: 3x/10'/30''" />
                    </div>
                    <div class="md:col-span-3">
                        <flux:input wire:model="djj" label="DJJ" placeholder="Ex: 140" />
                    </div>
                    <div class="md:col-span-6">
                        <flux:input wire:model="keterangan" label="Keterangan" placeholder="Ex: Tetesan 10 tpm" />
                    </div>
                </div>
            </div>
