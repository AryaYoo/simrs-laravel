<div class="flex flex-col gap-6 pb-8" x-data="{ activeTab: 'input' }">
            ...
            <div>
                <nav class="text-xs text-neutral-400 mb-0.5">
                    ...
                    <a href="{{ route('modul.rawat-inap.perawatan-tindakan', str_replace('/', '-', $no_rawat)) }}"
                        wire:navigate class="hover:underline">Perawatan</a>
                    <span class="mx-1">/</span>
    ...
    {{-- Patient Info Banner --}}
    <div
        class="bg-[#4C5C2D] text-white p-4 rounded-xl shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span
                        class="font-mono text-xs bg-white/20 px-2 py-0.5 rounded">{{ $regPeriksa->no_rkm_medis }}</span>
                </div>
                ...
                <div class="text-xs text-white/80 mt-1 flex flex-wrap items-center gap-3">
                    <span class="flex items-center gap-1">
                        <flux:icon name="calendar" class="w-3 h-3" /> {{ $regPeriksa->umurdaftar }}
                        {{ $regPeriksa->sttsumur }}
                    </span>
                    @if($regPeriksa->kamarInap->first())
                        <span class="flex items-center gap-1">
                            <flux:icon name="home" class="w-3 h-3" />
                            {{ $regPeriksa->kamarInap->first()->kamar->bangsal->nm_bangsal ?? '-' }} &middot;
                            {{ $regPeriksa->kamarInap->first()->kd_kamar ?? '-' }}
                        </span>
                    @endif
                </div>
            </div>
        ...
        <div
            class="text-left md:text-right text-sm border-t md:border-t-0 md:border-l border-white/20 pt-3 md:pt-0 md:pl-4">
    ...
    <div class="border-b border-neutral-200 dark:border-neutral-700">
        <nav class="flex gap-6">
            ...
            <button @click="activeTab = 'riwayat'"
                ...
                @if(count($history) > 0)
                    <span
                        class="ml-1 px-1.5 py-0.5 rounded-full bg-[#4C5C2D]/10 text-[#4C5C2D] text-[10px] font-black">{{ count($history) }}</span>
                @endif
            </button>
        </nav>
    </div>
    ...
    <div x-show="activeTab === 'input'" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div
                    ...
                    <div
                        class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex items-center gap-2">
                        <flux:icon name="pencil-square" class="w-5 h-5 text-[#4C5C2D]" />
                        <h3 class="font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-tight text-sm">
                            Informasi Permintaan</h3>
                    </div>
                    <div class="p-5 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <flux:field>
                                ...
                                <div class="flex items-center gap-1.5">
                                    <flux:input type="number" wire:model="jam_permintaan_jam"
                                        class="text-center font-mono" min="0" max="23" :disabled="$auto_waktu" />
                                    <span class="text-neutral-300 font-bold">:</span>
                                    <flux:input type="number" wire:model="jam_permintaan_menit"
                                        class="text-center font-mono" min="0" max="59" :disabled="$auto_waktu" />
                                    <span class="text-neutral-300 font-bold">:</span>
                                    <flux:input type="number" wire:model="jam_permintaan_detik"
                                        class="text-center font-mono" min="0" max="59" :disabled="$auto_waktu" />
                                </div>
                            </flux:field>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <flux:field>
                                ...
                                <flux:input wire:model="predictedOrderNo" readonly
                                    class="bg-neutral-50 font-mono font-bold text-[#4C5C2D]" />
                                <flux:description class="text-[10px] mt-1 italic">Nomor ini bersifat sementara & akan
                                    dikunci saat simpan.</flux:description>
                            </flux:field>
                            <flux:field>
                                <flux:label>Dokter Perujuk (DPJP)</flux:label>
                                <div class="flex gap-2">
                                    <flux:input wire:model="nm_dokter_perujuk" readonly
                                        class="flex-1 bg-neutral-50 font-bold" />
                                    <button type="button" wire:click="openDokterModal"
                                        class="p-1.5 text-neutral-500 hover:text-[#4C5C2D] transition-colors border border-neutral-200 dark:border-neutral-700 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800">
                ...
                <div
                    ...
                    <div
                        ...
                        <h3 class="font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-tight text-sm">
                            Indikasi & Informasi Tambahan</h3>
                    </div>
                    <div class="p-5 space-y-5">
                        <flux:field>
                            <flux:label>Indikasi Pemeriksaan / Diagnosis Klinis</flux:label>
                            <flux:textarea wire:model="diagnosa_klinis" rows="2"
                                placeholder="Masukkan diagnosa klinis atau alasan pemeriksaan..." class="resize-none" />
                        </flux:field>
                        <flux:field>
                            <flux:label>Informasi Tambahan Permintaan Foto</flux:label>
                            <flux:textarea wire:model="informasi_tambahan" rows="2"
                                placeholder="Contoh: Thorax 2 posisi, Cito, dll..." class="resize-none" />
                        </flux:field>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                    ...
                    <div class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex flex-col md:flex-row md:items-center justify-between gap-3">
                        <div class="flex items-center gap-2">
                            <flux:icon name="photo" class="w-5 h-5 text-[#4C5C2D]" />
                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex items-center gap-1.5">
                                <span class="text-[10px] font-bold text-neutral-400 uppercase tracking-wider whitespace-nowrap">Per halaman</span>
                                <select wire:model.live="perPageRadiologi" class="text-xs border border-neutral-200 dark:border-neutral-700 rounded-lg px-2 py-1.5 bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 focus:ring-[#4C5C2D] focus:border-[#4C5C2D]">
                            ...
                            <div class="w-full md:w-72">
                                <flux:input wire:model.live.debounce.300ms="searchPemeriksaan" placeholder="Cari kode atau nama..." icon="magnifying-glass" />
                            </div>
                        </div>
                    </div>

                    {{-- Summary Bar (shows when items are selected) --}}
                    @if(count($selectedTests) > 0)
                        @php
                            $totalTarif = \App\Models\JnsPerawatanRadiologi::whereIn('kd_jenis_prw', $selectedTests)->sum('total_byr');
                        @endphp
                        <div class="px-4 py-3 bg-[#F1F5E9] dark:bg-[#4C5C2D]/10 border-b border-[#4C5C2D]/10 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="flex items-center gap-1.5">
                                    <flux:icon name="check-circle" class="w-4 h-4 text-[#4C5C2D]" />
                                    <span class="text-xs font-bold text-[#4C5C2D]">{{ count($selectedTests) }} pemeriksaan dipilih</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <span class="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Est. Tarif:</span>
                                <span class="text-sm font-black text-[#4C5C2D] font-mono tabular-nums">Rp {{ number_format($totalTarif, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @endif
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="text-[10px] text-neutral-500 uppercase bg-neutral-50 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 sticky top-0 z-10 font-bold tracking-wider">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-right">Tarif (Rp)</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                                @forelse ($pemeriksaanList as $pemeriksaan)
                                    ...
                                    <tr 
                                        ...
                                        <td class="px-4 py-3.5 text-right font-mono text-xs tabular-nums {{ $isSelected ? 'text-[#4C5C2D] font-bold' : 'text-neutral-600 dark:text-neutral-400' }}">{{ number_format($pemeriksaan->total_byr, 0, ',', '.') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-12 text-center text-neutral-400">
                                            ...
                                            <p class="text-sm font-medium">Pemeriksaan tidak ditemukan</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            <div class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                    ...
                    <div class="p-5">
                        @if(count($selectedTests) > 0)
                            <div class="space-y-2 mb-4">
                                @foreach($selectedTests as $kd)
                                    ...
                                    @if($item)
                                        <div
                                            class="flex items-center justify-between p-2.5 rounded-lg bg-[#F1F5E9] dark:bg-[#4C5C2D]/10 border border-[#4C5C2D]/10">
                                            <div>
                                                <p
                                                    class="text-xs font-bold text-neutral-800 dark:text-neutral-200 uppercase tracking-tight">
                                                    {{ $item->nm_perawatan }}</p>
                                                <p class="text-[10px] font-mono text-neutral-400">{{ $item->kd_jenis_prw }}</p>
                                            </div>
                                            <p class="text-xs font-mono font-bold text-neutral-600">Rp
                                                {{ number_format($item->total_byr, 0, ',', '.') }}</p>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                            <div
                                class="flex items-center justify-between p-3 rounded-lg bg-neutral-50 dark:bg-neutral-900/50 border border-neutral-100 dark:border-neutral-700">
                        ...
                        @else
                            <div class="py-8 text-center">
                                ...
                                <p class="text-xs font-bold text-neutral-400 uppercase tracking-wide">Belum ada pemeriksaan
                                    dipilih</p>
                ...
                <div class="bg-[#F1F5E9] dark:bg-[#4C5C2D]/10 rounded-xl border border-[#4C5C2D]/20 p-5">
                    <flux:button wire:click="save" variant="primary" icon="paper-airplane"
                        class="w-full !bg-[#4C5C2D] hover:!bg-[#3D4A24] !border-none text-white shadow-md font-bold py-3 h-auto text-sm flex items-center justify-center gap-2"
                        wire:loading.attr="disabled">
                        Kirim Permintaan Radiologi
                    </flux:button>
                    <p class="text-[10px] text-neutral-500 text-center mt-3">Pastikan data sudah benar sebelum mengirim
                        permintaan.</p>
                </div>
            </div>
        </div>
    ...
    {{-- ===== TAB 2: RIWAYAT ===== --}}
    <div x-show="activeTab === 'riwayat'" x-cloak x-transition:enter="transition ease-out duration-200"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
        <div
            class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden">
            <div
                class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <flux:icon name="clock" class="w-5 h-5 text-[#4C5C2D]" />
                    <h3 class="font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-tight text-sm">
                        Riwayat Permintaan Radiologi</h3>
                </div>
            ...
            @forelse($history as $item)
                <div class="border-b border-neutral-100 dark:border-neutral-700 last:border-b-0">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-shrink-0">
                                @if($item->tgl_sampel == '1000-01-01' || $item->tgl_sampel == '0000-00-00')
                                    <div
                                        class="w-10 h-10 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 flex items-center justify-center">
                                        <flux:icon name="clock" class="w-5 h-5 text-amber-500" />
                                    </div>
                                @else
                                    <div
                                        class="w-10 h-10 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 flex items-center justify-center">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-0.5">
                                    ...
                                    @if($item->tgl_sampel == '1000-01-01' || $item->tgl_sampel == '0000-00-00')
                                        <span
                                            class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-600 text-[9px] font-bold uppercase tracking-tight border border-amber-200">Menunggu</span>
                                    @else
                                        <span
                                            class="px-1.5 py-0.5 rounded bg-green-100 text-green-600 text-[9px] font-bold uppercase tracking-tight border border-green-200">Diproses</span>
                                    @endif
                                </div>
                        ...
                        <div class="flex items-center gap-2 ml-14 md:ml-0">
                            @if($item->tgl_sampel == '1000-01-01' || $item->tgl_sampel == '0000-00-00')
                                <flux:button variant="ghost" size="xs" icon="trash"
                                    class="text-rose-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg"
                    ...
                    <div class="px-5 pb-4 ml-14">
                        <div
                            class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-lg bg-neutral-50/80 dark:bg-neutral-900/30 border border-neutral-100 dark:border-neutral-700/50">
                            {{-- Dokter --}}
                            <div>
                                <p class="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Dokter
                                    Perujuk</p>
                                <p class="text-xs font-semibold text-neutral-700 dark:text-neutral-300">
                                    {{ $item->dokter->nm_dokter ?? '-' }}</p>
                            </div>
                            {{-- Indikasi --}}
                            <div>
                                <p class="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Indikasi
                                    Klinis</p>
                                <p class="text-xs text-neutral-600 dark:text-neutral-400 leading-snug">
                                    {{ $item->diagnosa_klinis ?: '-' }}</p>
                            </div>
                            {{-- Info Tambahan --}}
                            <div>
                                <p class="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Info
                                    Tambahan</p>
                                <p class="text-xs text-neutral-600 dark:text-neutral-400 leading-snug">
                                    {{ $item->informasi_tambahan ?: '-' }}</p>
                            </div>
                        </div>

                        {{-- Pemeriksaan Items --}}
                        <div class="mt-3">
                            <p class="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-2">Pemeriksaan
                                Diminta</p>
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($item->detailPemeriksaan as $detail)
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-[#F1F5E9] dark:bg-[#4C5C2D]/10 border border-[#4C5C2D]/10 text-[10px] font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-tight">
    ...
    <flux:modal wire:model="isDokterModalOpen" variant="flyout" class="w-full max-w-lg">
        ...
        <flux:input wire:model.live.debounce.300ms="searchDokterModal" icon="magnifying-glass"
            placeholder="Cari berdasarkan nama dokter..." autofocus class="mb-4" />

        <div class="overflow-y-auto border border-neutral-200 dark:border-neutral-700 rounded-lg"
            style="max-height: 60vh;">
            <table class="w-full text-sm text-left">
                <thead
                    class="text-[10px] text-neutral-500 uppercase bg-neutral-50 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 sticky top-0 z-10 font-bold tracking-wider">
                    <tr>
                ...
                <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                    @forelse($listDokter as $doc)
                        <tr class="hover:bg-[#F1F5E9] dark:hover:bg-[#4C5C2D]/10 transition-colors cursor-pointer group"
                            ...
                            <td
                                class="px-4 py-3 font-bold text-neutral-800 dark:text-neutral-100 group-hover:text-[#4C5C2D]">
                                {{ $doc['nm_dokter'] }}</td>
                        </tr>
                    @empty
                        <tr>
