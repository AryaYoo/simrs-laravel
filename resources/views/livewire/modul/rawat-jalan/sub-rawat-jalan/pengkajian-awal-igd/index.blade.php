<div class="flex flex-col gap-6 pb-24" x-data="{ isSubmitting: false }">
    ...
    <div class="sticky top-0 z-40 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-700 -mx-4 px-4 py-3 mb-2 flex items-center justify-between shadow-sm">
        <div class="flex items-center gap-3">
            <a href="{{ route('modul.rawat-jalan.perawatan-tindakan', str_replace('/', '-', $regPeriksa->no_rawat)) }}" wire:navigate class="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
        ...
        <div class="flex items-center gap-2">
            <flux:button href="{{ route('modul.rawat-jalan.perawatan-tindakan', str_replace('/', '-', $regPeriksa->no_rawat)) }}" wire:navigate variant="ghost" class="h-9 text-sm">Batal</flux:button>
    ...
    <div x-data="{
        ...
        <template x-for="s in sections" :key="s.id">
            <div class="group flex items-center justify-end gap-3">
                <span :class="activeSection === s.id ? 'opacity-100 text-[#4C5C2D] scale-100' : 'opacity-0 group-hover:opacity-100 text-neutral-400 scale-95 translate-x-2 group-hover:translate-x-0'"
                      class="text-[10px] font-black uppercase tracking-widest transition-all duration-300" x-text="s.label"></span>
                <button @click="scrollTo(s.id)"
                        :class="activeSection === s.id ? 'bg-[#4C5C2D] text-white scale-110 shadow-lg' : 'bg-white dark:bg-neutral-800 text-neutral-400 hover:text-[#4C5C2D] border border-neutral-200 dark:border-neutral-700 shadow-sm hover:scale-105'"
                        class="w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300">
                    <flux:icon ::name="s.icon" class="w-5 h-5" />
                </button>
            </div>
        </template>
    </div>
