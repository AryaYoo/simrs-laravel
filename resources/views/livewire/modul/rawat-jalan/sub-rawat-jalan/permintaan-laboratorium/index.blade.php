<div>
    ...
    <div class="flex items-center gap-1 mb-6 border-b border-neutral-200 dark:border-neutral-700">
        ...
        <button wire:click="$set('kategori', 'RIWAYAT')" class="px-6 py-3 text-sm font-bold transition-all border-b-2 {{ $kategori === 'RIWAYAT' ? 'border-[#4C5C2D] text-[#4C5C2D]' : 'border-transparent text-neutral-400 hover:text-neutral-600' }} flex items-center gap-2">
            Riwayat
            @if($hasNewHistory)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden flex flex-col h-[700px]">
                <div class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex flex-col gap-4">
                    ...
                    <flux:input wire:model.live.debounce.300ms="searchPemeriksaan" placeholder="Cari Kode atau Nama Pemeriksaan..." icon="magnifying-glass" class="!py-1.5 !text-xs" />
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-neutral-100 dark:divide-neutral-700">
                            @forelse($pemeriksaanList as $item)
                                <tr wire:click="toggleTest('{{ $item->kd_jenis_prw }}')" class="hover:bg-[#4C5C2D]/5 transition-colors group cursor-pointer">
                                    <td class="px-4 py-3 text-center">
                                        <flux:checkbox wire:model.live="selectedTests" value="{{ $item->kd_jenis_prw }}" class="accent-[#4C5C2D]" onclick="event.stopPropagation()" />
                                    </td>
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden flex flex-col h-[700px]">
                <div class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex flex-col gap-4">
                    ...
                    <flux:input wire:model.live.debounce.300ms="searchDetail" placeholder="Cari dalam detail pemeriksaan..." icon="magnifying-glass" class="!py-1.5 !text-xs" />
                </div>
                <div class="flex-1 overflow-auto">
                            ...
                            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-700">
                                @foreach($detailParameters->groupBy('kd_jenis_prw') as $kd_jenis_prw => $group)
                                    ...
                                    @foreach($group as $detail)
                                        <tr wire:click="toggleDetail('{{ $detail->id_template }}')" class="hover:bg-[#4C5C2D]/5 transition-colors group cursor-pointer">
                                            <td class="px-4 py-2 text-center">
                                                <flux:checkbox wire:model.live="selectedDetails" value="{{ (string)$detail->id_template }}" class="accent-[#4C5C2D]" onclick="event.stopPropagation()" />
                                            </td>
                                            <td class="px-4 py-2">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden flex flex-col">
                <div class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex flex-col gap-4">
                    ...
                    <flux:input wire:model.live.debounce.300ms="searchPemeriksaan" placeholder="Cari jenis pemeriksaan Patologi Anatomi..." icon="magnifying-glass" class="!py-1.5 !text-xs" />
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden flex flex-col h-[700px]">
                <div class="p-4 border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50 flex flex-col gap-4">
                    ...
                    <flux:input wire:model.live.debounce.300ms="searchPemeriksaan" placeholder="Cari Kode atau Nama Pemeriksaan MB..." icon="magnifying-glass" class="!py-1.5 !text-xs" />
                </div>
                <div class="flex-1 overflow-auto">
                    <table class="w-full text-left border-collapse">
                        <thead class="sticky top-0 bg-white dark:bg-neutral-800 shadow-sm z-10 font-bold text-neutral-500 dark:text-neutral-400 text-[10px] uppercase tracking-wider">
                ...
                <tbody class="divide-y divide-neutral-100 dark:divide-neutral-700">
                    @forelse($this->pemeriksaanHistory as $history)
                        <tr class="hover:bg-neutral-50/50 dark:hover:bg-neutral-900/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-400 font-medium">{{ $history->dokter->nm_dokter ?? '-' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('modul.rawat-jalan.permintaan-lab.cetak', ['no_rawat' => str_replace('/', '-', $no_rawat), 'noorder' => $history->noorder]) }}" 
                                       wire:navigate
                                    ...
                                    @if($history->tgl_sampel == '1000-01-01' || $history->tgl_sampel == '0000-00-00')
                                        <button type="button" onclick="confirm('Apakah Anda yakin ingin membatalkan permintaan ini?') || event.stopImmediatePropagation()" wire:click="batalPermintaan('{{ $history->noorder }}')" class="p-2 text-red-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all" title="Batal Permintaan">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-20 text-center text-neutral-400 italic text-sm">Belum ada riwayat permintaan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <flux:modal name="dokter_modal" wire:model="isDokterModalOpen" variant="flyout" class="w-full max-w-lg">
        <div class="mb-4 text-lg font-bold flex items-center gap-2">
            ...
            @if(count($listDokter) > 0)
                <ul class="divide-y divide-neutral-100 dark:divide-neutral-800">
                    @foreach($listDokter as $dr)
                        <li class="group">
                            <button type="button" wire:click="selectDokter('{{ $dr['kd_dokter'] }}', '{{ addslashes($dr['nm_dokter']) }}')" class="w-full text-left px-5 py-4 hover:bg-[#F1F5E9] dark:hover:bg-neutral-800 transition-all duration-200">
                                <div class="font-semibold text-sm text-neutral-800 dark:text-neutral-200 group-hover:text-[#4C5C2D]">{{ $dr['nm_dokter'] }}</div>
                                <div class="text-[10px] text-neutral-400 font-mono mt-1">{{ $dr['kd_dokter'] }}</div>
                            </button>
                        </li>
                    @endforeach
                </ul>
