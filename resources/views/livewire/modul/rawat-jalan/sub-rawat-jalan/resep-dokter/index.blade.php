<div>
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-xs text-white/80 mt-1 flex flex-wrap items-center gap-3">
                    ...
                    <span class="flex items-center gap-1"><flux:icon name="home" class="w-3 h-3"/> {{ $regPeriksa->poliklinik->nm_poli ?? '-' }}</span>
                </div>
            </div>
        </div>
    ...
    <div class="flex items-center gap-1 mb-6 border-b border-neutral-200 dark:border-neutral-700">
        <button wire:click="$set('tab', 'input')" class="px-6 py-3 text-sm font-bold transition-all border-b-2 {{ $tab === 'input' ? 'border-[#4C5C2D] text-[#4C5C2D]' : 'border-transparent text-neutral-400 hover:text-neutral-600' }}">Input Resep</button>
        <button wire:click="$set('tab', 'riwayat')" class="px-6 py-3 text-sm font-bold transition-all border-b-2 {{ $tab === 'riwayat' ? 'border-[#4C5C2D] text-[#4C5C2D]' : 'border-transparent text-neutral-400 hover:text-neutral-600' }}">Riwayat Permintaan</button>
    </div>
    ...
    @elseif($tab === 'riwayat')
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden flex flex-col">
            <div class="px-6 py-4 border-b border-neutral-100 dark:border-neutral-700 flex items-center justify-between bg-neutral-50 dark:bg-neutral-800/50">
                <div class="flex items-center gap-2">
                    <flux:icon name="clock" class="w-5 h-5 text-[#4C5C2D] dark:text-[#8CC7C4]" />
                    <h3 class="font-bold text-neutral-800 dark:text-neutral-200 uppercase tracking-tight">Riwayat Permintaan Resep Pasien</h3>
                </div>
                <span class="px-3 py-1 bg-[#4C5C2D] text-white text-[10px] font-bold rounded-full uppercase tracking-widest">{{ count($savedResep) }} Total Resep</span>
            </div>
            
            <div class="p-6 space-y-6">
                @forelse($savedResep as $resep)
                    <div class="border border-neutral-200 dark:border-neutral-700 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
                        <div class="bg-gradient-to-r from-neutral-50 to-white dark:from-neutral-800/80 dark:to-neutral-900 px-4 py-3 border-b border-neutral-200 dark:border-neutral-700 flex flex-wrap gap-4 items-center justify-between">
                            <div class="flex items-center gap-6">
                                <div class="flex flex-col">
                                    <span class="text-[9px] uppercase font-bold text-neutral-400 tracking-widest">No. Resep</span>
                                    <span class="bg-[#4C5C2D]/10 text-[#4C5C2D] dark:text-[#8CC7C4] px-2 py-0.5 rounded font-mono text-xs font-bold border border-[#4C5C2D]/20 mt-1">#{{ $resep->no_resep }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-[9px] uppercase font-bold text-neutral-400 tracking-widest">Waktu Peresepan</span>
                                    <div class="text-xs text-neutral-700 dark:text-neutral-300 font-bold mt-1 flex items-center gap-1.5">
                                        <flux:icon name="calendar" class="w-3.5 h-3.5 opacity-50"/>
                                        {{ \Carbon\Carbon::parse($resep->tgl_peresepan)->format('d/m/Y') }} <span class="text-neutral-400 mx-1">|</span> {{ $resep->jam_peresepan }}
                                    </div>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-[9px] uppercase font-bold text-neutral-400 tracking-widest">Dokter Peresep</span>
                                    <div class="text-xs text-neutral-700 dark:text-neutral-300 font-bold mt-1 flex items-center gap-1.5">
                                        <flux:icon name="user" class="w-3.5 h-3.5 opacity-50"/>
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" 
                                    ...
                                    "
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white border border-red-200 rounded-lg transition-all text-xs font-bold uppercase tracking-wider group">
                                    <flux:icon name="trash" class="w-3.5 h-3.5 group-hover:animate-pulse" /> Hapus Resep
                                </button>
                            </div>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-xs text-left">
                                <thead class="bg-neutral-50/50 dark:bg-neutral-800/30 text-[10px] uppercase font-black text-neutral-400 border-b border-neutral-100 dark:border-neutral-800">
                                    <tr>
                                        <th class="px-6 py-3 font-bold tracking-widest">Kode</th>
                                        <th class="px-6 py-3 font-bold tracking-widest">Nama Obat / Barang</th>
                                        <th class="px-6 py-3 font-bold tracking-widest text-center w-24">Jumlah</th>
                                        <th class="px-6 py-3 font-bold tracking-widest">Aturan Pakai</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800/50">
                                    @foreach($resep->detail as $detail)
                                        <tr class="bg-white dark:bg-neutral-900 hover:bg-[#F1F5E9]/50 dark:hover:bg-[#4C5C2D]/10 transition-colors">
                                            <td class="px-6 py-3 font-mono text-neutral-400">{{ $detail->kode_brng }}</td>
                                            <td class="px-6 py-3 font-bold text-neutral-800 dark:text-neutral-200">
                                                {{ $detail->barang->nama_brng ?? $detail->kode_brng }}
                                            </td>
                                            <td class="px-6 py-3 text-center align-middle">
                                                <span class="px-3 py-1 bg-neutral-100 dark:bg-neutral-800 rounded-full font-black text-neutral-700 dark:text-neutral-300">{{ $detail->jml }}</span>
                                            </td>
                                            <td class="px-6 py-3 text-neutral-600 dark:text-neutral-400 italic font-medium">
                                                "{{ $detail->aturan_pakai }}"
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                @empty
                    <div class="text-center py-20 bg-neutral-50 dark:bg-neutral-900/50 rounded-2xl border-2 border-dashed border-neutral-200 dark:border-neutral-700">
                        <div class="flex flex-col items-center justify-center text-neutral-400">
                            <flux:icon name="archive-box" class="w-16 h-16 mb-4 opacity-10" />
                            <p class="text-lg font-bold">Belum Ada Riwayat Resep</p>
                            <p class="text-sm opacity-60">Tidak ditemukan permintaan resep pada kunjungan ini.</p>
                        </div>
                    </div>
                @endforelse
            </div>
