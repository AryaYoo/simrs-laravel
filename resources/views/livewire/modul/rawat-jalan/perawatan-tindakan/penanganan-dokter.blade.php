<div class="animate-in fade-in duration-300">
    ...
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2">
            <flux:icon name="user-plus" class="w-5 h-5 text-[#4C5C2D]" />
            <h3 class="text-sm font-semibold text-neutral-700 dark:text-neutral-200">Data Penanganan Dokter</h3>
        </div>
        <div class="flex items-center gap-3">
            @php $drTindakan = $this->allTindakan->where('type', 'dr'); @endphp
            <span class="text-xs text-neutral-400 bg-neutral-100 dark:bg-neutral-700 px-2 py-1 rounded-full">{{ $drTindakan->count() }} catatan</span>
                            ...
                            <button type="button" @click="
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        $wire.deleteTindakan('{{ $item['type'] }}', '{{ $item['kd_jenis_prw'] }}', '{{ $item['tgl_perawatan'] }}', '{{ $item['jam_rawat'] }}', '{{ $item['kd_staff_dr'] }}');
                                    }
                                });
                            "
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="5" class="text-center py-12">
                        <flux:icon name="user-plus" class="w-10 h-10 mx-auto mb-3 text-neutral-200 dark:text-neutral-700" />
                        <p class="text-xs text-neutral-400 uppercase tracking-widest font-medium italic">Belum ada penanganan dokter yang tercatat</p>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
        ...
        <div class="flex flex-col h-full bg-white dark:bg-neutral-900" x-data="{ showFormStaff: null }">
            ...
            <div class="px-6 py-4 border-b border-neutral-200 dark:border-neutral-700 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-bold text-neutral-800 dark:text-neutral-100 uppercase">Input Penanganan Dokter</h2>
                </div>
            ...
            <div x-show="detailModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-90 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-100 dark:border-neutral-800/50 bg-gradient-to-r from-[#F1F5E9] to-white dark:from-[#4C5C2D]/10 dark:to-neutral-900 flex-shrink-0">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="font-bold text-neutral-800 dark:text-neutral-100 text-base uppercase tracking-tight">Detail Penanganan Dokter</h2>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" @click="
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $wire.deleteTindakan(detail.type, detail.kd_jenis_prw, detail.tgl_perawatan, detail.jam_rawat, detail.type == 'dr' ? detail.kd_staff_dr : '');
                                }
                            });
                        "
