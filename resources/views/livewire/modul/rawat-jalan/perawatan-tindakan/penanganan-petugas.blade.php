<div class="animate-in fade-in duration-300">
    ...
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-semibold text-neutral-700 dark:text-neutral-200">Data Penanganan Petugas</h3>
        </div>
        <div class="flex items-center gap-3">
            @php $prTindakan = $this->allTindakan->where('type', 'pr'); @endphp
            <span class="text-xs text-neutral-400 bg-neutral-100 dark:bg-neutral-700 px-2 py-1 rounded-full">{{ $prTindakan->count() }} catatan</span>
                            ...
                            <button type="button" @click="
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        $wire.deleteTindakan('{{ $item['type'] }}', '{{ $item['kd_jenis_prw'] }}', '{{ $item['tgl_perawatan'] }}', '{{ $item['jam_rawat'] }}', '{{ $item['kd_staff_pr'] }}');
                                    }
                                });
                            "
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="5" class="text-center py-12">
                        <flux:icon name="users" class="w-10 h-10 mx-auto mb-3 text-neutral-200 dark:text-neutral-700" />
                        <p class="text-xs text-neutral-400 uppercase tracking-widest font-medium italic">Belum ada penanganan petugas yang tercatat</p>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <flux:modal wire:model="tindakanCreateModalOpen" variant="flyout" class="w-full max-w-2xl">
        <div class="flex flex-col h-full bg-white dark:bg-neutral-900" x-data="{ showFormStaff: null }">
            ...
            <div class="px-6 py-4 border-b border-neutral-200 dark:border-neutral-700 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="p-2 rounded-lg bg-blue-600/10 text-blue-600">
                        <flux:icon name="plus-circle" class="w-5 h-5" />
                    </div>
                    <h2 class="text-lg font-bold text-neutral-800 dark:text-neutral-100 uppercase">Input Penanganan Petugas</h2>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <div x-show="showFormStaff === 'pr'" @click.away="showFormStaff = null"
                            ...
                            <div class="mt-3 space-y-1 max-h-48 overflow-y-auto">
                                @forelse($this->petugasList as $pr)
            ...
            <div class="p-6 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-800/50">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-lg font-bold text-neutral-800 dark:text-neutral-100 flex items-center gap-2 uppercase tracking-wide">
                        <flux:icon name="magnifying-glass" class="w-5 h-5 text-blue-600" />
                        Cari Tindakan Paramedis
                    </h2>
                    <flux:modal.close>
            ...
            <div class="flex-1 overflow-y-auto p-0 min-h-[50vh]">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                        @forelse($this->tindakanList as $t)
                            <tr class="hover:bg-[#F1F5E9]/50 dark:hover:bg-neutral-800 transition-colors cursor-pointer group" wire:click="previewTindakan('{{ $t->kd_jenis_prw }}', '{{ addslashes($t->nm_perawatan) }}')">
                                <td class="p-4 pl-6 text-xs font-mono text-neutral-500 group-hover:text-blue-600">{{ $t->kd_jenis_prw }}</td>
                                <td class="p-4">
                                    <span class="text-sm font-bold text-neutral-700 dark:text-neutral-200 uppercase group-hover:text-blue-600">{{ $t->nm_perawatan }}</span>
                                </td>
    ...
    <template x-teleport="body">
        <div x-show="detailModalOpen" @keydown.escape.window="closeDetailModal()" class="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            <div x-show="detailModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-90 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                ...
                {{-- Header --}}
                <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-100 dark:border-neutral-800/50 bg-gradient-to-r from-blue-50 to-white dark:from-blue-600/10 dark:to-neutral-900 flex-shrink-0">
                    <div class="flex items-center gap-3">
                        <div class="p-2.5 rounded-2xl bg-blue-600/10 dark:bg-blue-600/30 border border-blue-600/20">
                            <flux:icon name="identification" class="w-5 h-5 text-blue-600" />
                        </div>
                        <div>
                            <h2 class="font-bold text-neutral-800 dark:text-neutral-100 text-base uppercase tracking-tight">Detail Penanganan Petugas</h2>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                        ...
                        <div class="lg:col-span-12">
                            <section class="bg-white dark:bg-neutral-800/80 p-6 rounded-2xl border border-neutral-100 dark:border-neutral-700 shadow-sm h-full flex flex-col">
                                ...
                                <div class="bg-blue-50/50 dark:bg-blue-600/10 p-5 rounded-2xl border border-blue-200/50 mb-6">
                                    <p class="text-[10px] font-bold text-blue-600 uppercase mb-1 tracking-widest">Penanganan / Tindakan :</p>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" @click="
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $wire.deleteTindakan(detail.type, detail.kd_jenis_prw, detail.tgl_perawatan, detail.jam_rawat, detail.type == 'pr' ? detail.kd_staff_pr : '');
                                }
                            });
                        "
